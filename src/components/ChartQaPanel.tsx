...
const SUGGESTIONS = ["career", "dasha", "relationships", "children", "timing"] as const;

async function fetchQaUsage(): Promise<QaUsage> {
  const res = await fetch("/api/qa", { credentials: "include" });
  ...
  while (true) {
    ...
    for (const part of parts) {
      ...
      if (payload.type === "meta") {
        ...
        throw new Error(payload.message ?? "Q&A failed");
      }
    }
  }
...
export function ChartQaPanel({ chart }: ChartQaPanelProps) {
  const t = useTranslations("reading.qa");
  const locale = useLocale() as Locale;
  const [messages, setMessages] = useState<QaMessage[]>([]);
  const [input, setInput] = useState("");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [usage, setUsage] = useState<QaUsage>({
    used: 0,
    limit: 8,
    hasApiKey: false,
    loaded: false,
  ...
  async function sendQuestion(question: string) {
    ...
    try {
      const res = await fetch("/api/qa", {
        ...
        body: JSON.stringify({ chart, locale, messages: nextMessages }),
      });
      ...
      const meta = await consumeQaSse(res, (chunk) => {
